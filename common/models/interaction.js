...
//-------------------------------------------------------------
  Interaction.sockets = function (cb) {
    var sockets = Interaction.app.interactionService.listInteractions();
    sockets = _.isEmpty(sockets) ? { sockets: {} } : sockets;
    return cb(null, sockets);
  }

  Interaction.remoteMethod('sockets', {
    accepts: [],
    returns: { root: true },
    http: { path: '/sockets', verb: 'post' }
  })
}
